...
void *threadFunction(void *th_number){
  ...
  if(*number == 2){
    ...
    if(*number == 3){
    int t1 = 1;
    if(pthread_create(&th[0], NULL, threadFunction, (void *)&t1)!=0){
                  ...
                  default: //parent, p2
                      ...
                      switch (pid[6]) {
                        ...
                        case 0: //child of p2, process p6
                            info(BEGIN, 6, 0);
                            int t[] = {2, 3, 4};
                            if(pthread_create(&th[2], NULL, threadFunction, (void *)&t[0])!=0){
                              perror("Cannot create threads");
                              exit(1);
                            }

                            if(pthread_create(&th[1], NULL, threadFunction, (void *)&t[1])!=0){
                              perror("Cannot create threads");
                              exit(1);
                            }

                            if(pthread_create(&th[3], NULL, threadFunction, (void *)&t[2])!=0){
                              perror("Cannot create threads");
                              exit(1);
                            }
